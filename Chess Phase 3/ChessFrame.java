...
public class ChessFrame extends JFrame implements ChessBoardPanel.GameEventListener {
    ...
    private void buildMenuBar() {
        ...
        // ---- AI menu ----
        gameMenu.addSeparator();
        aiToggleItem = menuItem("Play vs Computer", null);
        aiToggleItem.addActionListener(e -> toggleAI());
        gameMenu.add(aiToggleItem);

        JMenu diffMenu = menu("AI Difficulty");
        for (ChessAI.Difficulty d : ChessAI.Difficulty.values()) {
            JMenuItem dItem = menuItem(d.toString(), null);
            dItem.addActionListener(e -> {
                if (ai != null) ai.setDifficulty(d);
                else ai = new ChessAI(d);
                JOptionPane.showMessageDialog(ChessFrame.this,
                    "AI difficulty set to: " + d, "Difficulty", JOptionPane.INFORMATION_MESSAGE);
            });
            diffMenu.add(dItem);
        }
        bar.add(diffMenu);
    ...
    /** Toggles the AI opponent on/off. */
    private void toggleAI() {
        aiEnabled = !aiEnabled;
        if (aiEnabled) {
            if (ai == null) ai = new ChessAI(ChessAI.Difficulty.MEDIUM);
            aiToggleItem.setText("Play vs Human");
            gameState.newGame();
            refreshViews();
            maybeDoAIMove();
        } else {
            aiToggleItem.setText("Play vs Computer");
            gameState.newGame();
            refreshViews();
        }
    }
    ...
    /**
     * If AI is enabled and it is the AI's turn, computes and executes
     * the best move after a short delay so the human can see the board.
     */
    private void maybeDoAIMove() {
        if (!aiEnabled || ai == null) return;
        if (gameState.getCurrentTurn() != ai.getColor()) return;
        if (gameState.isCheckmate() || gameState.isStalemate()) return;

        javax.swing.Timer timer = new javax.swing.Timer(350, e -> {
            if (!aiEnabled) return;
            ...
            if (move != null) {
                ...
                if (gameState.isCheckmate()) {
                    onCheckmate(gameState.getCurrentTurn().opposite());
